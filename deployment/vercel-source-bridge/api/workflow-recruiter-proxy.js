...
function handles(rawPath) { const path = String(rawPath || '').replace(/^\/+|\/+$/g, ''); return path === 'recruiter-proof' || path === 'recruiter-proof/index.html' || path === 'data/recruiter-proof.json' || path === '__recruiter_proof_verify'; }
function repositoryName(repoUrl) { const prefix = 'https://github.com/'; requireValue(typeof repoUrl === 'string' && repoUrl.startsWith(prefix), `recruiter_repo_boundary:${repoUrl}`); const repository = repoUrl.slice(prefix.length).replace(/\/$/, ''); requireValue(/^GlacierEQ\/[A-Za-z0-9_.-]+$/.test(repository), `recruiter_repo_invalid:${repository}`); return repository; }
function freshnessWeight(ageDays) { requireValue(Number.isInteger(ageDays) && ageDays >= 0, 'recruiter_invalid_age'); if (ageDays <= 30) return 1; if (ageDays <= 90) return 0.85; if (ageDays <= 180) return 0.65; if (ageDays <= 365) return 0.4; return 0.2; }
...
async function fetchJson(url, fetchImpl = fetch) {
  ...
  try {
    const response = await fetchImpl(url, { headers: { accept: 'application/vnd.github+json', 'user-agent': `GlacierEQ-${RELEASE}/1.0`, 'x-github-api-version': '2022-11-28' }, signal: controller.signal, redirect: 'error' });
...
async function mapConcurrent(items, limit, mapper) {
  ...
  async function worker() { while (true) { const index = cursor; cursor += 1; if (index >= items.length) return; results[index] = await mapper(items[index], index); } }
...
function selectVerificationRun(payload, repository, defaultBranch = null) {
  const source = VERIFICATION_SOURCES[repository];
  if (!source) return null;
  const enforceIdentity = typeof defaultBranch === 'string' && defaultBranch.length > 0;
  const runs = Array.isArray(payload?.workflow_runs) ? payload.workflow_runs : [];
  const candidates = runs.filter((run) => {
    const pathBound = !enforceIdentity || source.workflow_paths === null || (typeof run?.path === 'string' && source.workflow_paths.includes(run.path));
    const branchBound = !enforceIdentity || runMatchesBranchPolicy(run, source, defaultBranch);
    return run?.status === 'completed' && run?.conclusion === 'success'
      && source.workflow_names.includes(run?.name) && pathBound && branchBound
      && typeof run?.head_sha === 'string' && /^[a-f0-9]{40}$/.test(run.head_sha)
      && typeof run?.updated_at === 'string' && !Number.isNaN(Date.parse(run.updated_at));
  });
...
function verifyExactRunIdentity(run, selected, repository, defaultBranch) {
  ...
  requireValue(Number.isInteger(run?.id) && run.id > 0 && run.id === selected.id, 'recruiter_exact_run_id_mismatch');
  ...
  if (source.workflow_paths !== null) requireValue(typeof run?.path === 'string' && source.workflow_paths.includes(run.path), 'recruiter_exact_run_path_mismatch');
  return { id: run.id, name: run.name, path: run.path || null, head_branch: run.head_branch, event: run.event, head_sha: run.head_sha, updated_at: run.updated_at, html_url: run.html_url };
}
...
async function deriveFreshness(topology, { asOf = new Date(), fetchImpl = fetch } = {}) {
  ...
  for (const flow of topology.flows || []) for (const step of flow.steps || []) systems.set(step.system.id, step.system);
  ...
  await mapConcurrent(repositories, FETCH_CONCURRENCY, async (repository) => {
    if (!VERIFICATION_SOURCES[repository]) { repositoryState.set(repository, { error: 'verification_source_not_registered' }); return; }
    try {
      ...
      if (!selected) { repositoryState.set(repository, { error: 'registered_verification_run_not_found' }); return; }
      requireValue(Number.isInteger(selected.id) && selected.id > 0, 'recruiter_selected_run_id_missing');
      const exact = await fetchJson(`${GITHUB_API}/repos/${repository}/actions/runs/${selected.id}`, fetchImpl);
      repositoryState.set(repository, { defaultBranch, run: verifyExactRunIdentity(exact, selected, repository, defaultBranch) });
    } catch (error) {
      repositoryState.set(repository, { error: `repository_verification_unavailable:${error instanceof Error ? error.message : String(error)}` });
    }
  });

  const entries = []; const missing = [];
  for (const [systemId, system] of orderedSystems) {
    const repository = repositoryName(system.repo); const state = repositoryState.get(repository);
    if (!state || state.error) { missing.push({ id: systemId, repository, reason: state?.error || 'repository_verification_state_missing' }); continue; }
    const run = state.run; const verifiedAt = new Date(run.updated_at); const ageDays = Math.floor((asOf.getTime() - verifiedAt.getTime()) / 86400000);
    if (ageDays < 0) { missing.push({ id: systemId, repository, reason: 'verification_timestamp_in_future' }); continue; }
    const weight = freshnessWeight(ageDays);
    entries.push({ id: systemId, repository, commit_sha: run.head_sha, verified_at: verifiedAt.toISOString(), age_days: ageDays, freshness_weight: weight, state: freshnessState(weight), verification_workflow: run.name, verification_workflow_path: run.path, verification_branch: run.head_branch, verification_event: run.event, verification_run_id: run.id, verification_url: run.html_url, default_branch: state.defaultBranch });
  }
  const core = { schema: 'glaciereq.public-evidence-freshness.v2', as_of: asOf.toISOString(), topology_receipt_sha256: topology.receipt_sha256, verification_source_policy: 'registered workflow name + exact workflow path when declared + branch/event policy + exact run readback; missing or unverifiable proof receives zero ranking credit', fetch_policy: { concurrency: FETCH_CONCURRENCY, timeout_ms: FETCH_TIMEOUT_MS }, entries, missing_systems: missing };
  return { ...core, receipt_sha256: receipt(core) };
}

async function loadLiveFreshness(topology) {
  const now = Date.now();
  if (liveFreshnessCache && liveFreshnessCache.topologyReceipt === topology.receipt_sha256 && liveFreshnessCache.expiresAt > now) return liveFreshnessCache.value;
  if (liveFreshnessPromise && liveFreshnessPromise.topologyReceipt === topology.receipt_sha256) return liveFreshnessPromise.promise;
  const promise = deriveFreshness(topology, { asOf: new Date(now) }).then((value) => { liveFreshnessCache = { topologyReceipt: topology.receipt_sha256, expiresAt: now + FRESHNESS_CACHE_MS, value }; return value; }).finally(() => { liveFreshnessPromise = null; });
  liveFreshnessPromise = { topologyReceipt: topology.receipt_sha256, promise }; return promise;
}

function rankFlows(topology, role, freshness) {
  const weights = ROLE_WEIGHTS[role]; requireValue(weights, `recruiter_unknown_role:${role}`);
  const freshnessById = new Map(freshness.entries.map((entry) => [entry.id, entry]));
  const ranked = topology.flows.map((flow) => {
    let staticRoleScore = 0; let freshnessAdjustedRoleScore = 0; const proofWeights = [];
    const proofPoints = flow.steps.map((step) => {
      const systemId = step.system.id; const roleWeight = weights[systemId] || 0; const evidence = freshnessById.get(systemId); const weight = evidence ? evidence.freshness_weight : 0; const weightedContribution = Math.round(roleWeight * weight * 1e6) / 1e6;
      staticRoleScore += roleWeight; freshnessAdjustedRoleScore += weightedContribution; proofWeights.push(weight);
      return { system_id: systemId, contribution: step.transition || '', evidence: step.system.evidence, current_ceiling: step.system.limit, repository: step.system.repo, role_weight: roleWeight, freshness_weight: weight, freshness_state: evidence?.state || 'unverified', age_days: evidence?.age_days ?? null, commit_sha: evidence?.commit_sha ?? null, verified_at: evidence?.verified_at ?? null, verification_workflow: evidence?.verification_workflow ?? null, verification_workflow_path: evidence?.verification_workflow_path ?? null, verification_branch: evidence?.verification_branch ?? null, verification_event: evidence?.verification_event ?? null, verification_run_id: evidence?.verification_run_id ?? null, verification_url: evidence?.verification_url ?? null, weighted_contribution: weightedContribution };
    });
    const breadthBonus = Math.min(new Set(flow.steps.map((step) => step.system.id)).size, 4); const breadthFactor = proofWeights.length ? proofWeights.reduce((sum, value) => sum + value, 0) / proofWeights.length : 0; const adjustedBreadth = Math.round(breadthBonus * breadthFactor * 1e6) / 1e6; const score = Math.round((freshnessAdjustedRoleScore + adjustedBreadth) * 1e6) / 1e6;
    return { flow_id: flow.id, name: flow.name, intent: flow.intent || '', score, static_role_score: staticRoleScore, freshness_adjusted_role_score: Math.round(freshnessAdjustedRoleScore * 1e6) / 1e6, breadth_bonus: breadthBonus, freshness_adjusted_breadth_bonus: adjustedBreadth, proof_points: proofPoints };
  });
  ranked.sort((a, b) => b.score - a.score || a.flow_id.localeCompare(b.flow_id)); return ranked;
}
...
module.exports = async function workflowRecruiterProxy(req, res) {
  const path = requestPath(req); if (path === '__recruiter_proof_verify') return verify(res); if (!handles(path)) { res.statusCode = 404; return res.end('Not found'); }
  const role = requestUrl(req).searchParams.get('role') || 'recruiter'; if (!ROLE_WEIGHTS[role]) return send(res, 400, 'application/json; charset=utf-8', JSON.stringify({ schema: SCHEMA, status: 'INVALID_ROLE', role, allowed_roles: Object.keys(ROLE_WEIGHTS) }), 'no-store');
  try { const topology = await workflowTopologyProxy.loadTopology(); const proof = await buildPublicRecruiterProof(topology, role); if (path === 'data/recruiter-proof.json') return send(res, 200, 'application/json; charset=utf-8', JSON.stringify(proof, null, 2)); return send(res, 200, 'text/html; charset=utf-8', renderHtml(proof)); }
  catch (error) { return send(res, 503, 'application/json; charset=utf-8', JSON.stringify({ schema: SCHEMA, status: 'FAIL_CLOSED', role, error: error instanceof Error ? error.message : String(error) }, null, 2), 'no-store'); }
};
module.exports.handles = handles;
module.exports.constants = { RELEASE, LEGACY_RELEASE_MARKER, SCHEMA, VERIFY_SCHEMA, ROLE_WEIGHTS, VERIFICATION_SOURCES, FETCH_CONCURRENCY, FRESHNESS_CACHE_MS };
