...
const RELEASE = 'V23-ALGERIAN-DISPLAY';
const VERIFY_SCHEMA = 'glaciereq.v23-algerian-display-verification.v1';
...
async function verifyTypography(res) {
  ...
  let homepage = null;
  try {
    const [v22Response, cssBody, homeResponse] = await Promise.all([
      captureEstate({ url: '/?path=__v22_verify' }),
      loadTypographyCss(),
      captureEstate({ url: '/?path=index.html' }),
    ]);
    try { v22 = JSON.parse(v22Response.body.toString('utf8')); } catch {}
    if (v22Response.status !== 200 || v22?.status !== 'PASS' || v22?.schema !== 'glaciereq.v22-estate-intelligence-verification.v1') {
      errors.push('v22_verifier_failed');
    }
    css = {
    ...
    if (homepage.status !== 200 || homepage.stylesheet_count !== 1 || !homepage.script_free) {
  ...
  const payload = Buffer.from(JSON.stringify({
    ...
    inherited_v22: v22 ? { schema: v22.schema, status: v22.status } : null,
    css,
    homepage,
    truth_boundary: {
...
module.exports = async function typographyProxy(req, res) {
  ...
  if (rawPath === '__v23_verify') return verifyTypography(res);
