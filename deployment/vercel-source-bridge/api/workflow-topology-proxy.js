...
async function verify(res) {
  try {
    ...
    send(res, 200, 'application/json; charset=utf-8', JSON.stringify({ schema: VERIFY_SCHEMA, status: 'PASS', release: RELEASE, portfolio_commit: PORTFOLIO_COMMIT, donor_commit: DONOR_COMMIT, contraction_commit: CONTRACTION_COMMIT, flow_count: topology.flows.length, edge_count: topology.edges.length, system_count: allIds.size, receipt_sha256: topology.receipt_sha256, public_contract: { scripts: 0, inline_styles: 0, machine_json: true, server_filters: true } }, null, 2), 'no-store');
