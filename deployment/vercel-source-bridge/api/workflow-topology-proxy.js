...
function buildTopology(portfolio) {
  ...
  const flows = FLOW_BLUEPRINTS.map((blueprint) => ({
    ...
    steps: blueprint.systemIds.map((id, index) => ({
      ordinal: index + 1,
      transition: blueprint.transitions[index],
      system: systems.get(id),
    })),
  }));

  const edges = flows.flatMap((flow) => flow.steps.slice(0, -1).map((step, index) => ({
    flow_id: flow.id,
    from: step.system.id,
    to: flow.steps[index + 1].system.id,
    transition: flow.steps[index + 1].transition,
  })));
  const core = {
    schema: SCHEMA,
    release: RELEASE,
    ...
    restoration_lineage: {
      ...
      donor_path: 'site-v13/data/portfolio.graph.json#combination_flows',
      ...
      preserved_later_gains: ['script-free CSP', 'V23 systems atlas', 'V25 application compiler', 'V28 invention evidence runtime'],
    },
    flows,
    edges,
  };
...
async function loadTopology() {
  if (!topologyPromise) {
    topologyPromise = fetchPortfolio().then(buildTopology).catch((error) => {
...
function handles(rawPath) {
  ...
  return path === 'workflows'
    ...
    || path === 'data/workflow-topology.json'
    || path === 'assets/site.workflows.css'
    || path === '__workflow_topology_verify';
}
...
function selectFlows(req, topology) {
  ...
  const knownStates = new Set(topology.flows.flatMap((flow) => flow.steps.map((step) => step.system.state)));
  const invalid = [];
  ...
  return {
    flows: topology.flows.filter((flow) => {
      if (flowId && flow.id !== flowId) return false;
      if (systemId && !flow.steps.some((step) => step.system.id === systemId)) return false;
      if (state && !flow.steps.some((step) => step.system.state === state)) return false;
      return true;
    }),
    invalid: [],
  };
...
function securityHeaders(res) {
  ...
  res.setHeader('X-GlacierEQ-Workflow-Source', PORTFOLIO_COMMIT);
  res.setHeader('X-PSYSOCX-Release', RELEASE);
}

function send(res, status, type, body, cache = 'public, max-age=0, must-revalidate') {
...
async function verify(res) {
  try {
    ...
    const allIds = new Set(topology.flows.flatMap((flow) => flow.steps.map((step) => step.system.id)));
    requireValue(topology.flows.length === FLOW_BLUEPRINTS.length, 'workflow_verify_flow_count');
    requireValue(topology.edges.length >= 8, 'workflow_verify_edge_count');
    requireValue(allIds.size >= 7, 'workflow_verify_system_diversity');
    requireValue(/^[a-f0-9]{64}$/.test(topology.receipt_sha256), 'workflow_verify_receipt');
    const payload = {
      ...
      system_count: allIds.size,
      receipt_sha256: topology.receipt_sha256,
      public_contract: { scripts: 0, inline_styles: 0, machine_json: true, server_filters: true },
    };
    send(res, 200, 'application/json; charset=utf-8', JSON.stringify(payload, null, 2), 'no-store');
  } catch (error) {
...
module.exports = async function workflowTopologyProxy(req, res) {
  ...
  if (path === 'data/workflow-topology.json') {
    try {
      const topology = await loadTopology();
      return send(res, 200, 'application/json; charset=utf-8', JSON.stringify(topology, null, 2));
    } catch (error) {
      return send(res, 503, 'application/json; charset=utf-8', JSON.stringify({ schema: SCHEMA, status: 'FAIL_CLOSED', error: error instanceof Error ? error.message : String(error) }, null, 2), 'no-store');
    }
  }
  if (path === 'workflows' || path === 'workflows/index.html') {
    try {
      const topology = await loadTopology();
      return send(res, 200, 'text/html; charset=utf-8', renderHtml(topology, selectFlows(req, topology)));
    } catch (error) {
      return send(res, 503, 'application/json; charset=utf-8', JSON.stringify({ schema: SCHEMA, status: 'FAIL_CLOSED', error: error instanceof Error ? error.message : String(error) }, null, 2), 'no-store');
    }
  }
