...
const required = new Set(['job_app_helix', 'akos', 'doctor_strange', 'tower_of_babel', 'sigma_glue', 'pro_code_runtime', 'job_application']);
...
if ((snapshot.flagships ?? []).length < 7) throw new Error(`flagship depth regressed: ${(snapshot.flagships ?? []).length}`);
if (missing.length) throw new Error(`required public flagship systems missing: ${missing.join(',')}`);
...
console.log(JSON.stringify({ status: 'PASS', flagships: snapshot.flagships.length, required_systems: [...required], company_membership_required: false }, null, 2));
