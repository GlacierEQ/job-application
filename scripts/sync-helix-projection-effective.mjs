...
async function fetchJson(url, init) {
  ...
  if (!response.ok) {
    throw new Error(`effective company second-depth fetch failed ${response.status}: ${url}`);
  }
...
async function effectiveSecondDepth(url, init) {
  ...
  const indexUrl = `${rawBase}${OVERRIDE_INDEX_SUFFIX}`;
  const indexResponse = await nativeFetch(indexUrl, init);
  ...
  if (!indexResponse.ok) {
    throw new Error(`modular override index fetch failed ${indexResponse.status}`);
  }
