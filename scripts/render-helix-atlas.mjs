...
function companyBlock(company) {
  ...
  const repos = company.repositories
    .map((repo) => {
      ...
      return `<li><a href="${repoUrl(repo.repository)}" target="_blank" rel="noopener">${escapeHtml(name)}</a><span>${escapeHtml(repo.level)} · ${escapeHtml(repo.promotion_state)}</span></li>`;
    })
    .join("");
  const applicableFlagships = Array.isArray(company.applicable_flagships) ? company.applicable_flagships : [];
  const fallback = applicableFlagships.length
    ? `<p class="atlas-applicable"><strong>Applicable personal flagships:</strong> ${applicableFlagships.map(escapeHtml).join(" · ")}</p>`
    : "";
