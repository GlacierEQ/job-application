...
const POWER_LAYERS = [
  ...
  ["Mission + autonomy", ["spacex", "palantir", "anduril", "tesla", "waymo", "zoox", "blue_origin", "rocket_lab", "nasa", "robotics"]],
...
function constellationNode(company, index) {
  ...
  return `<a class="atlas-star star-p${index} ${state}" href="${companyRoute(company.company_id)}" aria-label="Open ${escapeHtml(company.display_name)} company intelligence" title="${escapeHtml(company.display_name)} · ${evidenceLabel(state)}"><span class="star-core"></span><span class="star-label">${escapeHtml(company.display_name)}</span></a>`;
}

function directoryCard(company) {
  const state = evidenceState(company);
  return `<a class="atlas-directory-item" href="${companyRoute(company.company_id)}"><span><strong>${escapeHtml(company.display_name)}</strong><small>${escapeHtml(company.track_state)}</small></span><b class="evidence-state ${state}">${evidenceLabel(state)}</b></a>`;
}
...
<head>
  ...
  <meta name="description" content="Independent GlacierEQ technical alignment dossier for ${escapeHtml(company.display_name)}: current evidence, innovation context, machine contract, and evolution mesh.">
...
<div class="signal-bar"><div class="shell signal-inner"><span class="signal-live">COMPANY INTELLIGENCE · ${escapeHtml(evidenceLabel(state).toUpperCase())}</span><span>independent study · no affiliation implied</span></div></div>
...
<main id="main">
<section class="company-hero"><div class="shell company-hero-grid"><div><p class="eyebrow">INDEPENDENT COMPANY LENS · ${escapeHtml(evidenceLabel(state).toUpperCase())}</p><h1>${escapeHtml(company.display_name)}</h1><p class="lead">${escapeHtml(company.recruiter_thesis)}</p><div class="company-role-chips">${roleChips}</div></div><aside class="card company-state-card"><span class="evidence-state ${state}">${evidenceLabel(state)}</span><strong>${company.repositories.length}</strong><p>direct public evidence ${company.repositories.length === 1 ? "repository" : "repositories"}</p><small>${escapeHtml(company.track_state)}</small></aside></div></section>
<section class="section company-layer" id="recruiter"><div class="shell"><div class="layer-heading"><span>01 · RECRUITER</span><h2>What matters to this operating environment.</h2></div><div class="company-two-col"><article class="card"><h3>Alignment thesis</h3><p>${escapeHtml(company.recruiter_thesis)}</p><p>${escapeHtml(evidenceSummary)}</p></article><article class="card boundary-card"><h3>Truth boundary</h3><p>${escapeHtml(company.non_affiliation)}</p></article></div></div></section>
<section class="section alt company-layer" id="master"><div class="shell"><div class="layer-heading"><span>02 · MASTER</span><h2>Innovation, architecture, and the second-depth gate.</h2></div><div class="company-two-col"><article class="card"><h3>Role-specific engineering frame</h3><p>The current dossier identifies the domain and public evidence subset without pretending that company-level bottlenecks have already been proven. The next depth is code-path inspection plus live-role reconciliation: role → team problem → official evidence → repository → implementation → reproducible proof → bounded application claim.</p><h3>Current evidence</h3>${repos ? `<ul class="company-repo-ledger">${repos}</ul>` : `<p class="empty-state">No direct repository is recruiter-admitted yet. This is an engineering queue, not a reason to invent proof.</p>`}</article><article class="card aspiration-card"><p class="eyebrow">ASPIRATION</p><h3>Bring the implementation up to the claim.</h3><p>Use the present code as the baseline and the strongest useful, defensible capability as the target. Gaps are promoted into bounded repairs, tests, demos, benchmarks, and receipts before the claim is raised.</p><p><strong>Current gate:</strong> ${escapeHtml(company.gap_or_next_gate)}</p></article></div></div></section>
<section class="section company-layer" id="machine"><div class="shell"><div class="layer-heading"><span>03 · MACHINE</span><h2>Dense integration contract.</h2></div><div class="card machine-contract"><div class="machine-contract-head"><p>Compact wire view for agents and tooling. Full structured record is available beside this page.</p><a class="button ghost small" href="record.json">record.json</a></div><pre>${escapeHtml(machineWire(company, state))}</pre><p class="machine-hook">Consumer path: discover route → inspect record → follow public repository identities → verify native receipts/tests → respect gate and boundary.</p></div></div></section>
<section class="section alt company-layer" id="mesh"><div class="shell"><div class="layer-heading"><span>04 · MESH</span><h2>One node in the larger GlacierEQ system.</h2></div><div class="company-two-col"><article class="card"><h3>Relationships</h3>${meshLinks ? `<ul class="mesh-edge-list">${meshLinks}</ul>` : `<p class="empty-state">No direct public repository edge is admitted yet.</p>`}<p><a href="/mesh/">Open the estate-wide Mesh →</a></p></article><article class="card evolution-card"><p class="eyebrow">ASPIRATION &amp; EVOLUTION</p><h3>Promotion checklist</h3><ul class="evolution-list"><li>□ Verify a current role and its team constraints.</li><li>□ Inspect exact repository code paths at pinned heads.</li><li>□ Classify implementation and proof state separately.</li><li>□ Close the highest-ROI code/proof gap rather than lowering the aspiration.</li><li>□ Produce a reproducible proof artifact and claim ceiling.</li><li>□ Promote only after the proof survives adversarial review.</li></ul><p><strong>Next gate:</strong> ${escapeHtml(company.gap_or_next_gate)}</p></article></div></div></section>
...
async function loadSnapshot() {
  try {
    ...
    if (!snapshot || typeof snapshot !== "object" || Array.isArray(snapshot)) throw new Error("snapshot must contain an object");
    return snapshot;
  } catch (error) {
...
async function writeCompanyRoutes(companies) {
  ...
  for (const company of companies) {
    ...
    const state = evidenceState(company);
    await writeFile(path.join(directory, "index.html"), companyPage(company), "utf8");
    await writeFile(path.join(directory, "record.json"), stableJson(compactMachineRecord(company, state)), "utf8");
  }
}

async function main() {
  const snapshot = await loadSnapshot();
  if (snapshot.schema !== "glaciereq.public-portfolio-projection.v1") throw new Error("invalid Helix public projection schema");
  if (!Array.isArray(snapshot.flagships) || !Array.isArray(snapshot.companies)) throw new Error("Helix projection must contain flagship and company arrays");
  const companies = [...snapshot.companies].sort((a, b) => a.display_name.localeCompare(b.display_name));
  for (const company of companies) {
    companySlug(company.company_id);
    if (!Array.isArray(company.repositories)) throw new Error(`${company.company_id}: repositories must be an array`);
  }
...
<head>
  ...
  <meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
  <meta name="theme-color" content="#03080b">
  <meta name="description" content="Interactive Company Atlas for the GlacierEQ portfolio: a script-free constellation of governed company lenses, evidence states, system relationships, and real deep links.">
  <meta name="robots" content="index,follow">
...
<body>
<a class="skip" href="#main">Skip to content</a>
<div class="signal-bar"><div class="shell signal-inner"><span class="signal-live">HELIX ROOT TRUTH · COMPANY ATLAS</span><span>${companies.length} governed company lenses · ${publicMemberships} direct public memberships · zero client scripts</span></div></div>
...
<main id="main">
<section class="hero atlas-hero"><div class="shell"><p class="eyebrow">COMPANY INTELLIGENCE · REAL ROUTES · GOVERNED EVIDENCE</p><h1>Choose a star. <em>Follow the proof.</em></h1><p class="lead">The constellation is a navigation surface over the same Helix-governed portfolio graph. Every star is a real, keyboard-accessible link. Every company page separates current evidence from aspiration and carries Recruiter, Master, Machine, and Mesh depth without turning unfinished research into a conclusion.</p><div class="proof-strip" aria-label="Atlas scope"><div><b>${companies.length}</b><span>governed company lenses</span></div><div><b>${rich}</b><span>repository-rich</span></div><div><b>${seeded}</b><span>seeded</span></div><div><b>${scaffold}</b><span>scaffold</span></div></div><div class="actions"><a class="button primary" href="#constellation">Enter constellation</a><a class="button secondary" href="#power-map">Open power map</a><a class="button ghost" href="/data/helix-root.json">Inspect root snapshot</a></div></div></section>
<section id="constellation" class="section constellation-section"><div class="shell"><div class="section-head"><div><p class="eyebrow">CONSTELLATION MODE</p><h2>The ecosystem as a navigable field.</h2></div><p>Stars are positioned across three rings for dense scanning. Pointer precision is optional: the directory is the complete accessibility fallback and browser find provides instant text lookup without weakening the site’s no-script CSP.</p></div><div class="constellation-layout"><div class="constellation-stage" aria-label="Company constellation">${companies.map((company, index) => constellationNode(company, index)).join("")}<div class="constellation-core" aria-hidden="true"><span>GLACIEREQ</span><b>COMPANY<br>ATLAS</b></div><div class="orbit orbit-1" aria-hidden="true"></div><div class="orbit orbit-2" aria-hidden="true"></div><div class="orbit orbit-3" aria-hidden="true"></div></div><aside class="constellation-directory"><div class="directory-head"><p class="eyebrow">DIRECTORY</p><h3>All company lenses</h3><p>${evidenceLabel("repository-rich")} = multiple/direct advanced public evidence; Seeded = one direct public evidence repo; Scaffold = no company-specific repo admitted yet.</p></div><div class="atlas-directory">${companies.map(directoryCard).join("")}</div></aside></div></div></section>
...
<footer class="site-footer"><div class="shell"><p><strong>Casey Barton · GlacierEQ</strong></p><p>Independent systems work. Company alignment does not imply affiliation, endorsement, employment, proprietary access, or production deployment.</p></div></footer>
  ...
  await writeCompanyRoutes(companies);
  console.log(`Company Atlas rendered: ${path.relative(ROOT, ATLAS_OUTPUT)} + ${companies.length} company routes`);
}
