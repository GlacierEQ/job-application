...
That gate is now closed for the recovery/provenance lane. The canonical portfolio contains `portfolio-proof/CAPABILITY_CLUSTER__IDEMPOTENT_RECOVERY_AND_PROVENANCE__2026-08-08.md` at status `BEHAVIOR_TESTED_MULTI_REPO_PATTERN`, with exact-revision behavioral receipts for Sigma Glue and ECHO plus a separately bounded AKOS equivalence proof.
...
### 2. Idempotent recovery + provenance-bound state — BEHAVIOR-TESTED MULTI-REPO PATTERN
...
**Fit:** long-running agent infrastructure must distinguish first execution from retry/replay, preserve stable task identity, reject contradictory reuse, maintain provenance across state changes, and leave durable evidence for reconciliation. This cluster now demonstrates that control pattern independently of the Anthropic-specific scheduler donor.
...
The recovery/provenance cluster is behavior-tested, not runtime-proven. Promotion beyond that ceiling requires exact-revision restart/replay/readback receipts for each promoted runtime assertion.

## Recruiter surface

> I build deterministic control layers around agentic work: dependency-safe scheduling, explicit resource ceilings, stable identities for retries, idempotent execution boundaries, integrity/provenance checks, visible failures, and durable receipts. The scheduler proof is pinned to 62/62 passing repository-native tests, and the recovery pattern is independently behavior-tested across canonical systems. That maps directly to AI reliability work around long-running agents without claiming Anthropic-scale deployment or production reliability.
...
One proof family governs dependency graphs, complete-task funding, resource ceilings, and explicit deferral. A second independent family binds stable identity to idempotent/replay-safe execution, provenance/integrity checks, and durable reconciliation evidence.
...
**Unproven boundary**  
Anthropic runtime integration, production SLOs, model safety/alignment research, sandboxing, large-scale distributed semantics, and a full persistent multi-session agent runtime remain outside the claim.
...
Anthropic AI Reliability fit
  ...
  PROOF FAMILY B -> idempotent recovery + provenance
    BEHAVIOR_TESTED_MULTI_REPO_PATTERN
    Sigma Glue @ 4a1ca8e5... -> exact-revision verify success
    ECHO @ d8727616... -> exact-revision ECHO CI success
...
The prior projection's deferred continuity/recovery gate is now materially closed by a canonical, behavior-tested multi-repository pattern. The stronger claim is still bounded to reliability-control alignment: it does not convert successful CI into production-runtime evidence and does not imply Anthropic affiliation, runtime equivalence, or deployment.