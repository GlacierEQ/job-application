...
- **Agent Coordinator:** deterministic dependency, capacity, priority, and shared-budget scheduling has an immutable historical **62/62 Python test receipt** at executable commit `87438f57bdfd2cb380730cf51140611963d7c95b`. The current repository head `2329fd6ff7f1722310ce01ff0a6bad42025832d4` carries a newer excellence gap receipt with `blocker=TESTS_FAIL` (`ran=9`), so the 62/62 result is retained only as historical pinned evidence and is **not** represented as current-head verification or promotion.
...
Converts task graphs into accountable assignments only after checking dependency order, role fit, capacity, stable priority, and shared token budgets. The scheduler's historical executable proof is pinned to commit `87438f57bdfd2cb380730cf51140611963d7c95b` with 62/62 passing tests; current head `2329fd6ff7f1722310ce01ff0a6bad42025832d4` is not promoted because its newer excellence receipt records `TESTS_FAIL`. This distinction prevents historical proof from being silently inherited by a changed head.
