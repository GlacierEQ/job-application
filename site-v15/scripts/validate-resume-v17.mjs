...
const [html, css, ats, atsMd, jsonText, manifestText, llms, pdf, docx] = await Promise.all([
  read(paths.html),
  read(paths.css),
  read(paths.ats),
  read(paths.atsMd),
...
for (const href of ['/assets/site.css', '/assets/site.systems.css', '/assets/resume.v17.css']) {
...
for (const exact of ['69/69', '166 + 19', '148/148', 'REVIEWED_EXECUTION_BLOCKED', 'PSYSOC-X V17']) {
  assert(html.includes(exact) || jsonText.includes(exact), `exact signal missing: ${exact}`);
}
...
assert(resume.projects.some(project => project.name === 'Microcode Governance' && project.keywords.includes('REVIEWED_EXECUTION_BLOCKED')), 'microcode boundary missing');
...
const result = {
  ...
  files: {
    ...
    [paths.css]: { bytes: Buffer.byteLength(css), sha256: sha256(Buffer.from(css)) },
